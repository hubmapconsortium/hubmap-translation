...
if [[ "$1" != "start" && "$1" != "stop" && "$1" != "down" ]]; then
    ...
    if [ "$1" = "start" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p search_api up -d
    elif [ "$1" = "stop" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p search_api stop
    elif [ "$1" = "down" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p search_api down
    fi
fi
